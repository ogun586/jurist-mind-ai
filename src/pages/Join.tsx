...
export default function Join() {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    const toastId = toast.loading('Checking your application…');
    ...
    try {
      ...
      if (existing) {
        toast.error('You already have a lawyer profile registered.', { id: toastId });
        return;
      }
      ...
      if (avatarFile) {
        const path = `${user.id}/avatar-${stamp}.${safeExt(avatarFile)}`;
        uploadJobs.push(
          supabase.storage.from('lawyer-assets').upload(path, avatarFile).then(({ error }) => {
            if (error) throw new Error(`Avatar upload failed: ${error.message}`);
            return { type: 'avatar' as const, path, file: avatarFile };
          })
        );
      }

      if (credentialFile) {
        const path = `${user.id}/bar-license-${stamp}.${safeExt(credentialFile)}`;
        uploadJobs.push(
          supabase.storage.from('lawyer-credentials').upload(path, credentialFile).then(({ error }) => {
            if (error) throw new Error(`Credential upload failed: ${error.message}`);
            return { type: 'credential' as const, path, file: credentialFile };
          })
        );
      }
      ...
      uploadResults.forEach((result) => {
        if (result.status === 'fulfilled') {
          if (result.value.type === 'avatar') {
            const { data } = supabase.storage.from('lawyer-assets').getPublicUrl(result.value.path);
      ...
      const socialMedia = [form.social_linkedin, form.social_twitter].filter(Boolean).join(' | ');
      const practiceAreasArray = Array.isArray(form.specialization)
        ? form.specialization.map((s) => s.trim()).filter(Boolean)
      ...
      // Direct insert into lawyers table. Do not send is_verified because it is generated by the database.
      const { data, error } = await supabase
        ...
        .insert({
          ...
          firm_name: form.account_type === 'firm' ? form.firm_name || null : null,
          ...
          bio_structured: { about: form.description, approach: '', case_studies: [] },
      ...
      if (credentialUpload && data?.id) {
        supabase.from('lawyer_credentials').insert({
          ...
          if (error) console.warn('Credential record warning:', error);
        });
      }
      ...
      console.error(e);
      toast.error(e.message || 'Failed to submit registration', { id: toastId });
    } finally {
      setSubmitting(false);
    }
  };
